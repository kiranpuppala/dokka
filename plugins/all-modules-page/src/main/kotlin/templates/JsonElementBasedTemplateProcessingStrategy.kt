...
abstract class BaseJsonNavigationTemplateProcessingStrategy(val context: DokkaContext) : TemplateProcessingStrategy {
    ...
    open fun canProcess(file: File): Boolean = file.extension == "json" && file.nameWithoutExtension == navigationFileNameWithoutExtension
        ...
        if (canProcess) {
            launch {
                withContext(Dispatchers.IO) {
                    kotlin.runCatching { parseJson<AddToSearch>(input.readText()) }.getOrNull()
                }?.let { command ->
    ...
    override suspend fun finish(output: File) {
        if(fragments.isNotEmpty()){
            val content = toJsonString(fragments.entries.flatMap { (moduleName, navigation) -> navigation.map { it.withResolvedLocation(moduleName) } }).toByteArray()
            withContext(Dispatchers.IO) {
                Files.write(
                    output.resolve("$path/$navigationFileNameWithoutExtension.json").toPath(),
                    content
                )
                ...
                fragments.keys.forEach {
                    withContext(Dispatchers.IO) {
                        Files.write(
                            output.resolve(it).resolve("$path/$navigationFileNameWithoutExtension.json").toPath(),
                            content
                        )
                    }
                }
    ...
    private suspend fun fallbackToCopy(input: File, output: File) {
        ...
        withContext(Dispatchers.IO) { Files.copy(input.toPath(), output.toPath()) }
    }
...
class NavigationSearchTemplateStrategy(val dokkaContext: DokkaContext) : BaseJsonNavigationTemplateProcessingStrategy(dokkaContext) {
    override val navigationFileNameWithoutExtension: String = "navigation-pane"
    override val path: String = "scripts"
}

class PagesSearchTemplateStrategy(val dokkaContext: DokkaContext) : BaseJsonNavigationTemplateProcessingStrategy(dokkaContext) {
